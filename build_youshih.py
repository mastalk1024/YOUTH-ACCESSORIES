...
<div class="toolbar">
  <div class="sw">
    ...
    <input type="text" id="search" placeholder="多關鍵字空格分隔＝同時符合，例：尚宏 保護套；-詞 可排除（跨所有分類）">
  ...
  <div class="scope">
    <span>建檔/更新日期：</span>
    <input type="date" id="d-from" style="font:inherit;padding:3px 6px;border:1px solid #cbd5e1;border-radius:6px">
    <span>～</span>
    <input type="date" id="d-to" style="font:inherit;padding:3px 6px;border:1px solid #cbd5e1;border-radius:6px">
    <button id="d-clr" onclick="clrDate()" style="display:none;padding:3px 9px;font-size:.78rem;border:1px solid #cbd5e1;border-radius:6px;background:#fff;cursor:pointer">清除日期</button>
  </div>
...
<div id="banner">
  ...
  <button onclick="clrAll()">清除搜尋</button>
</div>
...
// ── State ─────────────────────────────────────────────────────────────────────
let brand='ys', cur='', q='', sc='', sd=1, pg=1, dFrom='', dTo='';
const PS=100;
...
const SEARCH_KEYS = {{
  ys: {{nm:'品名', pno:'料號', bc:'barcode', vd:'廠商', dt:'建檔日期'}},
  wk: {{nm:'品名', pno:'料號', bc:'barcode', vd:'供應商', yno:'y料號', dt:'更新日期'}},
}};

// ── Helpers ───────────────────────────────────────────────────────────────────
function esc(s){{return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}}
// 正規化：全形英數/符號→半形、全形空白→半形、轉小寫，讓料號/條碼打全形也能命中
function norm(s){{return String(s||'').replace(/[\\uFF01-\\uFF5E]/g,ch=>String.fromCharCode(ch.charCodeAt(0)-0xFEE0)).replace(/\\u3000/g,' ').toLowerCase()}}
// 把搜尋字串拆成多個詞：空白分隔，AND 關係；以 - 開頭者為排除詞
function parseTerms(str){{
  return norm(str).split(/\\s+/).filter(Boolean).map(tok=>{{
    if(tok.length>1&&tok[0]==='-')return{{neg:true,w:tok.slice(1)}};
    return{{neg:false,w:tok}};
  }}).filter(t=>t.w);
}}
function hi(t,terms){{
  const et=esc(t);
  const pos=(terms||[]).filter(x=>!x.neg&&x.w).map(x=>x.w.replace(/[.*+?^${{}}()|[\\]\\\\]/g,'\\\\$&'));
  if(!pos.length)return et;
  const r=new RegExp('('+pos.join('|')+')','gi');
  return et.replace(r,'<mark>$1</mark>');
}}
...
// 依勾選欄位取出該筆可搜尋文字（已正規化）
function rowFields(r,s,sk){{
  const f=[];
  if(s.nm)f.push(norm(r[sk.nm]));
  if(s.pno)f.push(norm(r[sk.pno]));
  if(s.bc)f.push(norm(r[sk.bc]));
  if(s.vd)f.push(norm(r[sk.vd]));
  if(s.yno&&sk.yno)f.push(norm(r[sk.yno]));
  return f;
}}
// 多關鍵字：每個詞需至少命中一個勾選欄位(OR)，且所有詞都要命中(AND)；排除詞只要命中即淘汰
function matchRow(r,terms,s,sk){{
  const f=rowFields(r,s,sk);
  for(const t of terms){{
    const hit=f.some(x=>x.includes(t.w));
    if(t.neg){{if(hit)return false;}}
    else{{if(!hit)return false;}}
  }}
  return true;
}}
// 日期區間：對應品牌的建檔/更新日期(ISO字串可直接比大小)；有篩選但該筆無日期則排除
function matchDate(r,sk){{
  if(!dFrom&&!dTo)return true;
  const d=(r[sk.dt]||'').slice(0,10);
  if(!d)return false;
...
function render(){{
  ...
  const dateOn=!!(dFrom||dTo);
  const active=terms.length>0||dateOn;
  let rows, catCol=false;

  if(active){{
    rows=allRows().filter(r=>matchRow(r,terms,s,sk)&&matchDate(r,sk));
    catCol=true;
  }} else {{
    rows=D[cur]||[];
  }}
  ...
  // Update tabs
  document.querySelectorAll('.tab').forEach(t=>t.classList.toggle('active',!active&&t.dataset.cat===cur));

  // Banner
  const bn=document.getElementById('banner');
  if(active){{
    let lbl=q;
    if(dateOn){{const dl='日期 '+(dFrom||'…')+'～'+(dTo||'…');lbl=lbl?lbl+' · '+dl:dl;}}
    document.getElementById('bq').textContent=lbl;
  ...
  // Info
  document.getElementById('ib').textContent=active?tot.toLocaleString()+' 筆':'顯示 '+tot.toLocaleString()+' 筆';
  ...
  if(!sl.length){{
    tb.innerHTML=`<tr><td colspan="${{(catCol?1:0)+cols.length}}" class="empty">查無符合條件的產品</td></tr>`;
  }} else {{
    tb.innerHTML=sl.map(r=>{{
      ...
      const cells=cols.map(c=>{{
        ...
        else{{rendered=`<span class="${{c.cls}}">${{hi(c.fmt(v||''),terms)}}</span>`}}
...
function clrDate(){{
  dFrom=''; dTo='';
  const a=document.getElementById('d-from'),b=document.getElementById('d-to');
  if(a)a.value=''; if(b)b.value='';
  document.getElementById('d-clr').style.display='none';
  pg=1; render();
}}
function clrAll(){{ // 清除搜尋：關鍵字＋日期一起清
...
['sc-nm','sc-pno','sc-bc','sc-vd','sc-yno'].forEach(id=>{{const el=document.getElementById(id);if(el)el.addEventListener('change',()=>{{if(q||dFrom||dTo){{pg=1;render()}}}});}});
// 日期篩選
['d-from','d-to'].forEach(id=>{{const el=document.getElementById(id);if(el)el.addEventListener('change',()=>{{
  dFrom=document.getElementById('d-from').value; dTo=document.getElementById('d-to').value;
  document.getElementById('d-clr').style.display=(dFrom||dTo)?'inline-block':'none';
  pg=1; render();
}});}});
...
function findNameCol(keys) {{
  for(const p of ['品名','name','名稱']) {{
    const f = keys.find(k=>k.toLowerCase().includes(p.toLowerCase()));
    if(f) return f;
  }}
...
async function exportCostExcel() {{
  ...
  validResults.forEach(cf => {{
    ...
    const mainWs = XLSX.utils.aoa_to_sheet([origKeys, ...mainRows]);
